    ...
    if (err) {
      ...
      db.query(query, function (err, resultss, fields) {
        if (err) {
          ...
          const uniqueKriteria = Array.from(
            new Set(results.map((a) => a.id_kriteria))
          ).map((id_kriteria) => {
          ...
          for (let i = 0; i < hasilArrr.length; i++) {
            const max = hasilArrr[i].reduce(function (prev, current) {
              return prev.bobot_subkriteria > current.bobot_subkriteria
                ? prev
                : current;
            });
            maxArr.push(max);
          ...
          for (let i = 0; i < hasilArr.length; i++) {
            for (let j = 0; j < uniqueKaryawan.length; j++) {
              hasilArr[i][j].bobot_subkriteria =
                (hasilArr[i][j].bobot_subkriteria -
                  minArr[i].bobot_subkriteria) /
          ...
          for (let i = 0; i < uniqueKaryawan.length; i++) {
            ...
            for (let j = 0; j < hasilArr.length; j++) {
              nilai =
                nilai +
                hasilArr[j][i].bobot_subkriteria *
                  hasilArr[j][i].bobot_kriteria;
            }
          ...
          uniqueKaryawan.sort(
            (a, b) =>
              parseFloat(b.bobot_subkriteria) - parseFloat(a.bobot_subkriteria)
          );
